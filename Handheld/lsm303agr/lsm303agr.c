...
int lsm303agr_enable_interrupt_1(const lsm303agr_t *dev)
{
	...
	res += i2c_write_reg(DEV_I2C, DEV_ACC_ADDR,
                        LSM303AGR_REG_INT1_THS_A ,0x10, 0); //Set threshold = 250mg
	res += i2c_write_reg(DEV_I2C, DEV_ACC_ADDR,
...
int lsm303agr_enable_interrupt_2(const lsm303agr_t *dev)
{
    ...
    temp = LSM303AGR_CTRL6_A_I2_INT2;
    temp |= LSM303AGR_CTRL6_A_P2_ACT;

    res += i2c_write_reg(DEV_I2C, DEV_ACC_ADDR,
                        LSM303AGR_REG_CTRL6_A ,temp, 0); //Activity interrupt enable on INT2 pin
    ...
    //                    LSM303AGR_REG_CTRL5_A ,0x08, 0); //Interrupt latched
	res += i2c_write_reg(DEV_I2C, DEV_ACC_ADDR,
                        LSM303AGR_REG_INT2_THS_A ,0x10, 0); //Set threshold = 250mg
	res += i2c_write_reg(DEV_I2C, DEV_ACC_ADDR,
                        LSM303AGR_REG_INT2_DURATION_A ,0x00, 0); //Set minimum event duration

    uint8_t temp1 = {LSM303AGR_INT2_XHIE |
                     LSM303AGR_INT2_YHIE 
              };

    res += i2c_write_reg(DEV_I2C, DEV_ACC_ADDR,
                        LSM303AGR_REG_INT2_CFG_A ,temp1, 0); //Xl Yl enable
